...
static unsigned int processor(const char* const *  const text, const ssize_t n_strings);
...
{		
	...
	if (command == PUSH) 	return_code |= do_push_command(text, n_string, stk_ptr);
...
static unsigned int processor(const char* const *  const text, const ssize_t n_strings)
{
	start_processor_stack();

	unsigned int return_code = 0;

	for (ssize_t n_string = 0; n_string < n_strings; n_string++)
	{
		return_code |= do_command(text, n_string, &stk);
